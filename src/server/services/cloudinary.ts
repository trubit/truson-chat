...
class CloudinaryService {
  private getFolder(userId: string, type: string): string {
    const map: Record<string, string> = {
      image:      `truson/users/${userId}/images`,
      video:      `truson/users/${userId}/videos`,
      audio:      `truson/users/${userId}/audio`,
      voice_note: `truson/users/${userId}/voice-notes`,
      document:   `truson/users/${userId}/documents`,
      sticker:    `truson/stickers`,
      gif:        `truson/gifs`,
      raw:        `truson/users/${userId}/files`,
    };
    return map[type] ?? `truson/users/${userId}/misc`;
  }
  ...
  async upload(
    ...
    const result: UploadApiResponse = await cloudinary.uploader.upload(filePath, uploadOpts);

    let thumbnailUrl: string | undefined;
    if (resourceType === 'image') {
      thumbnailUrl = cloudinary.url(result.public_id, {
        width: 400, height: 400, crop: 'fill', quality: 'auto', format: 'webp', secure: true,
      });
    } else if (resourceType === 'video' && !opts.mimeType.startsWith('audio/')) {
      thumbnailUrl = cloudinary.url(result.public_id, {
        resource_type: 'video', format: 'jpg', width: 480, crop: 'scale', quality: 'auto', secure: true,
      });
    }
  ...
  getOptimizedUrl(publicId: string, opts: TransformOptions = {}, resourceType: ResourceType = 'image'): string {
  ...
  getThumbnailUrl(publicId: string, width = 300, height = 300): string {
    return cloudinary.url(publicId, {
      width, height, crop: 'fill', quality: 'auto', format: 'webp', secure: true,
    });
  }

  getSignedUrl(publicId: string, expiresInSeconds = 3600): string {
    const expiresAt = Math.floor(Date.now() / 1000) + expiresInSeconds;
    return cloudinary.url(publicId, { sign_url: true, type: 'authenticated', expires_at: expiresAt, secure: true });
  }
}
