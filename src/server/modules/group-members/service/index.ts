...
export class GroupMemberService {
  async getMembers(
    viewerId: string,
    groupId: string,
    opts: { page: number; limit: number; role?: string },
  ): Promise<{ members: GroupMemberSummary[]; total: number; hasMore: boolean }> {
    const viewing = await groupRepository.findMember(groupId, viewerId);
    if (!viewing || viewing.status !== 'active')
  ...
  async updateRole(
    ...
    if (role === MEMBER_ROLES.OWNER && actor.role !== MEMBER_ROLES.OWNER)
    ...
    if (role === MEMBER_ROLES.OWNER) {
      await GroupMemberModel.findByIdAndUpdate(actor._id, {
        $set: { role: MEMBER_ROLES.ADMIN },
      }).exec();
    }
  ...
  async kickMember(actorId: string, groupId: string, targetUserId: string): Promise<void> {
    ...
    if (!target || target.status !== 'active')
      throw new AppError('Member not found', 404, 'NOT_FOUND');
    if (target.role === MEMBER_ROLES.OWNER)
      throw new AppError('Cannot kick the group owner', 400, 'BAD_REQUEST');
  ...
  async banMember(
    ...
    if (target?.role === 'owner')
      throw new AppError('Cannot ban the group owner', 400, 'BAD_REQUEST');
  ...
  async getBannedMembers(
    actorId: string,
    groupId: string,
  ): Promise<{ userId: string; reason?: string; bannedBy: string; expiresAt?: string }[]> {
    const actor = await groupRepository.findMember(groupId, actorId);
    if (!actor || !([MEMBER_ROLES.OWNER, MEMBER_ROLES.ADMIN] as string[]).includes(actor.role)) {
      throw new AppError('Insufficient permissions', 403, 'FORBIDDEN');
    }
