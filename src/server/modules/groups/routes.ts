...
import {
  createGroupSchema, updateGroupSchema, groupQuerySchema,
  sendGroupMessageSchema, editGroupMessageSchema, reactGroupMessageSchema,
  groupMessageQuerySchema, markGroupReadSchema, pinMessageSchema,
...
router.get('/discover', (req, _res, next) => {
  try { validate(groupQuerySchema, req.query); next(); } catch (e) { next(e); }
...
// Messages
router.get('/:id/messages', (req, _res, next) => {
  try { validate(groupMessageQuerySchema, { ...req.query, groupId: String(req.params.id) }); next(); } catch (e) { next(e); }
