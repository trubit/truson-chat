...
export class GroupController {
  ...
  async getMessages(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const q = req.query as { channelId?: string; limit?: string; before?: string; after?: string };
  ...
  async editMessage(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const data = await groupService.editMessage(req.user!.id, String(req.params.messageId), req.body.content);
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
  ...
  async reactToMessage(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const data = await groupService.reactToMessage(req.user!.id, String(req.params.messageId), req.body.emoji);
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
  ...
  async pinMessage(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const data = await groupService.pinMessage(req.user!.id, String(req.params.messageId), req.body.pin);
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
}
