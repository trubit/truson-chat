...
const GROUP_MSG_TYPES = ['text','image','video','audio','file','sticker','gif','location','contact','voice_note','system','announcement','pinned'] as const;

export const createGroupSchema = z.object({
  name:        z.string().min(1).max(100),
  description: z.string().max(1000).optional(),
  handle:      z.string().regex(/^[a-z0-9_]{3,32}$/, 'Handle must be 3-32 lowercase alphanumeric chars or underscores').optional(),
