...
export class GifService {
  async getTrending(limit = 20): Promise<GifItem[]> {
    ...
    if (!env.TENOR_API_KEY)
      throw new AppError('GIF service not configured', 503, 'SERVICE_UNAVAILABLE');

    try {
      const url = `${TENOR_BASE}/featured?key=${env.TENOR_API_KEY}&limit=${limit}&client_key=linkora&media_filter=gif`;
  ...
  async search(query: string, limit = 20): Promise<GifItem[]> {
    ...
    try {
      const url = `${TENOR_BASE}/search?key=${env.TENOR_API_KEY}&q=${encodeURIComponent(query)}&limit=${limit}&client_key=linkora&media_filter=gif`;
      const response = await fetch(url);
      if (!response.ok) throw new Error(`Tenor API returned ${response.status}`);
      const data = (await response.json()) as { results: TenorResult[] };
      return data.results.map(parseTenorResult);
