...
import type {
  AnnouncementSummary, CreateAnnouncementPayload, AnnouncementScope,
...
export class AnnouncementService {
  ...
  async getAnnouncements(_userId: string, opts: {
  ...
  async update(userId: string, announcementId: string, dto: Partial<CreateAnnouncementPayload> & { isPinned?: boolean }): Promise<AnnouncementSummary> {
    const doc = await AnnouncementModel.findById(new mongoose.Types.ObjectId(announcementId)).exec();
    if (!doc || doc.status === 'deleted') throw new AppError('Announcement not found', 404, 'NOT_FOUND');

    await this.requirePermission(userId, doc.scope, doc.groupId?.toString(), doc.communityId?.toString());
  ...
  private async requirePermission(userId: string, scope: AnnouncementScope, groupId?: string, communityId?: string): Promise<void> {
    if (scope === 'group' || scope === 'channel') {
      ...
      if (!m || !([MEMBER_ROLES.OWNER, MEMBER_ROLES.ADMIN, MEMBER_ROLES.MODERATOR] as string[]).includes(m.role)) {
