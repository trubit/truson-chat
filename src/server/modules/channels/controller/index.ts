...
export class ChannelController {
  ...
  async updateChannel(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const data = await channelService.updateChannel(req.user!.id, String(req.params.id), req.body);
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
  ...
  async removeMember(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      await channelService.removeChannelMember(req.user!.id, String(req.params.id), String(req.params.userId));
      res.status(200).json({ success: true, data: null });
    } catch (err) { next(err); }
  }
}
