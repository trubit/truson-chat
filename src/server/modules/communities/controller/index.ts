...
export class CommunityController {
  ...
  async getMyCommunities(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const data = await communityService.getMyCommunities(req.user!.id, Number(page), Number(limit));
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
  ...
  async discoverCommunities(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const data = await communityService.discoverCommunities({ page: Number(page), limit: Number(limit), q });
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
  ...
  async updateCommunity(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const data = await communityService.updateCommunity(req.user!.id, String(req.params.id), req.body);
      res.status(200).json({ success: true, data });
    } catch (err) { next(err); }
  }
  ...
  async removeGroup(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      await communityService.removeGroupFromCommunity(req.user!.id, String(req.params.id), String(req.params.groupId));
      res.status(200).json({ success: true, data: null });
    } catch (err) { next(err); }
  }
}
