...
import type {
  CommunitySummary, CommunityDetail,
  CreateCommunityPayload, UpdateCommunityPayload,
  GroupSummary,
...
function toDetail(c: ICommunity, myMembership?: ICommunityMember | null, groups?: GroupSummary[]): CommunityDetail {
  return {
    ...toSummary(c),
...
export class CommunityService {
  ...
  async createCommunity(userId: string, dto: CreateCommunityPayload): Promise<CommunityDetail> {
    ...
    await communityRepository.addMember({ communityId: community._id.toString(), userId, role: MEMBER_ROLES.OWNER });
  ...
  async getCommunity(communityId: string, viewerId?: string): Promise<CommunityDetail> {
    ...
    if (!community || community.deletedAt) throw new AppError('Community not found', 404, 'NOT_FOUND');
  ...
  async getMyCommunities(userId: string, page: number, limit: number): Promise<{ communities: CommunitySummary[]; total: number; hasMore: boolean }> {
  ...
  async discoverCommunities(opts: { page: number; limit: number; q?: string }): Promise<{ communities: CommunitySummary[]; total: number; hasMore: boolean }> {
    const { communities, total } = await communityRepository.findMany({ ...opts, type: 'public' });
    return { communities: communities.map(toSummary), total, hasMore: total > opts.page * opts.limit };
  }

  async updateCommunity(userId: string, communityId: string, dto: UpdateCommunityPayload): Promise<CommunityDetail> {
  ...
  async joinCommunity(userId: string, communityId: string): Promise<void> {
    const community = await communityRepository.findById(communityId);
    if (!community || community.deletedAt) throw new AppError('Community not found', 404, 'NOT_FOUND');
    if (community.type === 'private') throw new AppError('This community requires an invite', 403, 'INVITE_REQUIRED');
  ...
  async leaveCommunity(userId: string, communityId: string): Promise<void> {
    ...
    if (membership.role === MEMBER_ROLES.OWNER) throw new AppError('Owner cannot leave — transfer ownership first', 400, 'OWNER_CANNOT_LEAVE');
  ...
  async addGroupToCommunity(userId: string, communityId: string, groupId: string): Promise<void> {
    ...
    if (!groupMember || !([MEMBER_ROLES.OWNER, MEMBER_ROLES.ADMIN] as string[]).includes(groupMember.role)) {
      throw new AppError('You must be a group owner or admin to add it to a community', 403, 'FORBIDDEN');
    }

    await communityRepository.addGroup(communityId, groupId, userId);
  }

  async removeGroupFromCommunity(userId: string, communityId: string, groupId: string): Promise<void> {
    await this.requireRole(communityId, userId, [MEMBER_ROLES.OWNER, MEMBER_ROLES.ADMIN]);
  ...
  async requireRole(communityId: string, userId: string, roles: string[]): Promise<ICommunityMember> {
    const membership = await communityRepository.findMember(communityId, userId);
    if (!membership) throw new AppError('Not a community member', 403, 'NOT_MEMBER');
    if (!roles.includes(membership.role)) throw new AppError('Insufficient permissions', 403, 'FORBIDDEN');
    return membership;
  }
}
