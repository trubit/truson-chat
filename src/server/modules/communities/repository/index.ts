...
export class CommunityRepository {
  ...
  async findByUser(
    ...
    const skip = (opts.page - 1) * opts.limit;
    const memberships = await CommunityMemberModel.find({ userId: userOid, leftAt: null })
      .select('communityId')
      .lean()
      .exec();
  ...
  async addGroup(communityId: string, groupId: string, addedBy: string): Promise<void> {
    ...
    await GroupModel.findByIdAndUpdate(groupId, {
      $set: { communityId: new mongoose.Types.ObjectId(communityId) },
    }).exec();
  }
