...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await groupJoinRequestService.requestToJoin(req.user!.id, req.body.groupId, req.body.message);
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { groupId, page = '1', limit = '20' } = req.query as { groupId: string; page?: string; limit?: string };
...
router.patch('/:id/review', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await groupJoinRequestService.reviewRequest(req.user!.id, String(req.params.id), body.action, body.rejectReason);
    res.status(200).json({ success: true, data });
  } catch (err) { next(err); }
});
