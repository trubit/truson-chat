...
function toSummary(r: IGroupJoinRequest): GroupJoinRequestSummary {
  const user = r.userId as unknown as
    | { _id: mongoose.Types.ObjectId; displayName: string; avatarUrl?: string }
...
export class GroupJoinRequestService {
  ...
  async getPendingRequests(
  ...
  ): Promise<{ requests: GroupJoinRequestSummary[]; total: number; hasMore: boolean }> {
    ...
    const filter = {
      groupId: new mongoose.Types.ObjectId(groupId),
      status: 'pending' as import('../../../database/models/GroupJoinRequest.js').JoinRequestStatus,
    };
  ...
  async reviewRequest(
    ...
    if (!req || req.status !== 'pending')
      throw new AppError('Request not found or already reviewed', 404, 'NOT_FOUND');
    ...
    if (action === 'approve') {
      ...
      const existing = await groupRepository.findMember(
  ...
  async cancelRequest(userId: string, requestId: string): Promise<void> {
    ...
    if (req.userId.toString() !== userId)
      throw new AppError("Cannot cancel another user's request", 403, 'FORBIDDEN');
    if (req.status !== 'pending')
      throw new AppError('Request has already been reviewed', 400, 'BAD_REQUEST');
