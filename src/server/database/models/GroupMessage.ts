...
export type GroupMsgType =
  ...
  | 'voice_note' | 'system' | 'announcement' | 'pinned';
...
const mediaSchema = new Schema<IGroupMessageMedia>(
  {
    url: { type: String, required: true }, publicId: String, mimeType: String,
    size: Number, width: Number, height: Number, duration: Number,
    thumbnail: String, name: String, waveform: [{ type: Number }],
  },
  { _id: false },
);

const reactionSchema = new Schema<IGroupMessageReaction>(
  { emoji: { type: String, required: true, maxlength: 10 }, users: [{ type: Schema.Types.ObjectId, ref: 'User' }], count: { type: Number, default: 0 } },
  { _id: false },
);

const mentionSchema = new Schema<IGroupMention>(
  { userId: { type: Schema.Types.ObjectId, ref: 'User', required: true }, offset: { type: Number, required: true }, length: { type: Number, required: true } },
  { _id: false },
);
...
const groupMessageSchema = new Schema<IGroupMessage>(
  {
    ...
    type:      { type: String, enum: ['text','image','video','audio','file','sticker','gif','location','contact','voice_note','system','announcement','pinned'], default: 'text' },
