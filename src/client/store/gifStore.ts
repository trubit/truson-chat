...
export const useGifStore = create<GifState & GifActions>()((set) => ({
  ...
  setSearchResults: (gifs, query) => set({ searchResults: gifs, searchQuery: query, isSearching: false }),
  setSearching:     (v)           => set({ isSearching: v }),
  setLoadingTrend:  (v)           => set({ isLoadingTrend: v }),
  clearSearch:      ()            => set({ searchResults: [], searchQuery: '' }),
  reset:            ()            => set({ trending: [], searchResults: [], searchQuery: '', isSearching: false, isLoadingTrend: false }),
}));
