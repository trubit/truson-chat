        ...
        >
          ...
          {timeStr && (
            <Typography
              sx={{
                fontSize: 11,
                color: unread > 0 ? C.accent : C.txt3,
...
export default function ConversationList({
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
            ...
            <Typography
              sx={{ fontSize: 17, fontWeight: 800, color: C.txt1, letterSpacing: '-0.4px' }}
            >
              Messages
            </Typography>
          </Box>
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
        {isLoading && filtered.length === 0 ? (
          ...
          >
            ...
            <Typography
              sx={{ fontSize: 12.5, color: C.txt2, textAlign: 'center', lineHeight: 1.65 }}
            >
      ...
      {newChatOpen && <NewConversationDialog open={true} onClose={() => setNewChatOpen(false)} />}
    </Box>
  );
}
