import {
  Drawer, Box, Typography, Avatar, CircularProgress,
...
interface Props { open: boolean; onClose: () => void; groupId: string; }
...
export default function GroupMemberDrawer({ open, onClose, groupId }: Props) {
  ...
  return (
    <Drawer
      ...
      slotProps={{ paper: { sx: { width: 300, bgcolor: C.panel, borderLeft: `1px solid ${C.border}` } } }}
    >
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${C.border}` }}>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', py: 1 }}>
        ...
        {data?.members.map((m) => (
          <Box key={m.userId} sx={{
            ...
            <Avatar src={m.avatarUrl} sx={{ width: 38, height: 38, fontSize: 15, bgcolor: C.accent }}>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              {m.customTitle && (
                <Typography sx={{ color: C.txt2, fontSize: 12 }} noWrap>{m.customTitle}</Typography>
              )}
            </Box>
            <Chip
              label={m.role}
