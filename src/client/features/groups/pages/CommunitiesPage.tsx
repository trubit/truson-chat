...
import {
  ...
  Button, CircularProgress, TextField, InputAdornment, alpha, Chip,
} from '@mui/material';
...
function CommunityCard({ community, onView }: { community: CommunitySummary; onView: () => void }) {
  return (
    <Card sx={{ bgcolor: C.card, border: `1px solid ${C.border}`, borderRadius: '16px',
      transition: 'border-color 0.2s, box-shadow 0.2s',
      '&:hover': { borderColor: alpha(C.accent, 0.4), boxShadow: `0 4px 20px rgba(16,196,160,0.12)` },
    }}>
      <CardActionArea onClick={onView} sx={{ p: 2.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1.5 }}>
          <Avatar src={community.avatar?.url} sx={{ width: 52, height: 52, bgcolor: C.copper, fontWeight: 700, fontSize: 22 }}>
            {community.name[0]?.toUpperCase()}
          </Avatar>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography sx={{ color: C.txt1, fontWeight: 700, fontSize: 16 }} noWrap>{community.name}</Typography>
            {community.handle && (
              <Typography sx={{ color: C.txt2, fontSize: 12 }}>@{community.handle}</Typography>
            )}
          </Box>
          <Chip label={community.type} size="small"
            sx={{ bgcolor: alpha(community.type === 'public' ? C.accent : C.copper, 0.15),
              color: community.type === 'public' ? C.accent : C.copper,
              fontSize: 11, fontWeight: 700, border: `1px solid ${alpha(community.type === 'public' ? C.accent : C.copper, 0.3)}` }} />
        </Box>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Typography sx={{ color: C.txt2, fontSize: 12 }}>{community.memberCount.toLocaleString()} members</Typography>
          </Box>
        </Box>
      </CardActionArea>
    </Card>
...
export default function CommunitiesPage() {
  ...
  const { data: discoverData, isLoading: discoverLoading } = useDiscoverCommunities(tab === 'discover' ? q : undefined);

  const communities: CommunitySummary[] = tab === 'mine' ? (myData?.communities ?? []) : (discoverData?.data?.communities ?? []);
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 3 }}>
        {(['mine', 'discover'] as const).map((t) => (
          <Button key={t} onClick={() => setTab(t)}
            sx={{
              textTransform: 'none', borderRadius: '20px', px: 2.5, py: 0.75,
        ...
        <TextField fullWidth size="small" placeholder="Search communities…"
          ...
          sx={{
            ...
            '& .MuiOutlinedInput-root': {
              bgcolor: alpha('#fff', 0.04), borderRadius: '10px', color: C.txt1, fontSize: 14,
      ...
      {isLoading ? (
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 2 }}>
          {communities.map((community) => (
            <CommunityCard key={community._id} community={community} onView={() => navigate(`/communities/${community._id}`)} />
          ))}
        </Box>
      )}
