...
export interface ServerToClientEvents {
  ...
  'message:updated': (
    message: Pick<Message, '_id' | 'conversationId' | 'content' | 'editedAt'>,
  ) => void;
  ...
  'typing:start': (payload: {
    conversationId: string;
    userId: string;
    user: Pick<PublicUser, '_id' | 'displayName'>;
  }) => void;
  ...
  'group:message:updated': (
    message: Pick<GroupMessage, '_id' | 'groupId' | 'content' | 'isEdited' | 'editedAt'>,
  ) => void;
  'group:message:deleted': (payload: {
    messageId: string;
    groupId: string;
    deletedBy: string;
  }) => void;
  'group:message:reaction': (payload: {
    messageId: string;
    groupId: string;
    emoji: string;
    userId: string;
  ...
  'group:member:role_changed': (payload: {
    groupId: string;
    userId: string;
    role: GroupMemberSummary['role'];
    updatedBy: string;
  }) => void;
  'group:member:banned': (payload: {
    groupId: string;
    userId: string;
    bannedBy: string;
    reason?: string;
  ...
  'group:role:updated': (
    role: Partial<GroupRoleSummary> & { _id: string; groupId: string },
  ) => void;
  ...
  'group:announcement:updated': (
    announcement: Partial<AnnouncementSummary> & { _id: string },
  ) => void;
...
export interface ClientToServerEvents {
  ...
  'group:message:send': (
    ...
    callback: (r: SocketAck & { message?: GroupMessage }) => void,
  ) => void;
  'group:message:edit': (
    payload: EditGroupMessagePayload,
    callback: (r: SocketAck) => void,
  ) => void;
  'group:message:delete': (
    payload: DeleteGroupMessagePayload,
    callback: (r: SocketAck) => void,
  ) => void;
  'group:message:react': (
    payload: ReactGroupMessagePayload,
    callback: (r: SocketAck) => void,
  ) => void;
